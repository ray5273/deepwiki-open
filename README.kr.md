...
## 🧩 사내 OpenAI-Compatible API만 사용하기

사내망 안의 OpenAI-compatible API만 써서 DeepWiki를 돌리려면, 생성 모델과 임베딩 모델 설정을 별도 JSON으로 분리해서 쓰는 편이 가장 안전합니다.

### vLLM OpenAI-Compatible 게이트웨이

회사 vLLM 배포에서는 `local-config/vllm-openai-compatible/` 설정을 그대로 쓰면 됩니다. 이 설정은 하나의 사내 vLLM `/v1` base URL이 chat completions와 embeddings를 모두 제공한다는 전제입니다.

Docker Compose 예시:

```bash
cp .env.vllm.example .env
# VLLM_OPENAI_BASE_URL을 사내 vLLM /v1 endpoint로 수정
...
VLLM_OPENAI_BASE_URL=http://사내-vllm-gateway/v1
```

vLLM이 무인증 내부망으로 떠 있어도 OpenAI SDK는 API key 값 자체를 요구합니다. 그래서 vLLM 설정에는 `api_key: "EMPTY"`를 넣어둡니다. 나중에 vLLM을 `--api-key`로 띄우면 `generator.json`과 `embedder.json`의 값을 실제 키로 바꾸면 됩니다.

설정 파일의 모델명은 vLLM이 노출하는 이름과 맞아야 합니다. 기본 생성 모델은 `qwen3.5:9b`, 기본 임베딩 모델은 `bge-m3`입니다.

### 범용 사내 OpenAI-Compatible 게이트웨이

1. [generator.internal-openai-compatible.json.example](/Users/sanghyeok/workspace/deepwiki-open/api/config/generator.internal-openai-compatible.json.example) 파일을 복사해 별도 설정 디렉터리의 `generator.json`으로 둡니다.
2. [embedder.internal-openai-compatible.json.example](/Users/sanghyeok/workspace/deepwiki-open/api/config/embedder.internal-openai-compatible.json.example) 파일을 복사해 같은 디렉터리의 `embedder.json`으로 둡니다.
3. `.env`에 아래 값을 넣습니다.

```bash
DEEPWIKI_CONFIG_DIR=/절대경로/your-config-dir
INTERNAL_OPENAI_API_KEY=사내_LLM_API_KEY
INTERNAL_OPENAI_BASE_URL=http://사내-llm-gateway/v1
INTERNAL_EMBEDDING_API_KEY=사내_임베딩_API_KEY
INTERNAL_EMBEDDING_BASE_URL=http://사내-embedding-gateway/v1
```

핵심 포인트:

- `gemma`, `qwen`은 생성 모델이지 임베딩 모델이 아닙니다.
- 저장소 인덱싱을 하려면 `/v1/embeddings`를 지원하는 별도 임베딩 모델이 꼭 필요합니다.
- 사내 게이트웨이가 채팅과 임베딩을 둘 다 제공하면 두 base URL을 같은 값으로 써도 됩니다.
- 임베딩 모델명은 보통 `bge-m3`, `multilingual-e5-large`, `nomic-embed-text` 같은 계열이 더 적합합니다.
